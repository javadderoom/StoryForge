...
Strictly output a valid JSON object matching the requested schema. Do not enclose in markdown blocks if possible, or return clean JSON.${constraintLine}`);
    ...
    if (type === 'world') {
      ...
      schemaInstruction = `Schema: { "name": string, "speciesCategory": "beast"|"monstrosity"|"undead"|"elemental"|"flora"|"draconic"|"humanoid", "dangerLevel": 1|2|3|4|5, "rarity": "common"|"uncommon"|"rare"|"legendary", "habitatLocationIds": string[], "behavioralTactics": string, "weaknesses": string[], "resistances": string[], "harvestableLoot": [{ "itemId": string, "name": string, "dropRate": string }], "loreDescription": string } (If speciesCategory is "flora", this represents a botanical, fungal, or herbal organism: describe growth habits, magical/medicinal properties, seasonal blossoms, or harvesting conditions. Rarity reflects world population: "common" for abundant weeds/fauna, "uncommon" for scattered flora/predators, "rare" for scarce medicinal herbs/apex beasts, "legendary" for mythic or solitary specimens)`;
    } else if (type === 'deity') {
...
- "current" values represent a fully-rested state (current = max).)`;
      ...
      schemaInstruction = `Schema: { "name": string, "speciesCategory": "beast"|"monstrosity"|"undead"|"elemental"|"flora"|"draconic"|"humanoid", "habitatLocationName": string, "predatorPreyNiche": string, "nonCombatPacificationMethod": string, "pacificationReagents": string[], "preySpecies": string[], "predatorSpecies": string[], "alchemicalYields": [{ "reagentName": string, "rarity": "uncommon"|"rare"|"epic", "craftingUse": string }] } (Generate ecological food chain dynamics, behavioral tactics, non-lethal pacification methods, connected preySpecies and predatorSpecies names, and 1 to 3 harvestable alchemical / crafting reagents. IMPORTANT FOR PACIFICATION: In pacificationReagents, list the clean entity names of any plants, herbs, animal meats, or reagents required to pacify or bait the creature (e.g. ["نیلوفر مردابی"]). In nonCombatPacificationMethod, surround each required plant/meat/reagent in quotes like «نیلوفر مردابی». Prioritize reusing already-existing flora, beasts, or reagents from the world context when available. If speciesCategory is "flora", nonCombatPacificationMethod describes delicate botanical harvesting conditions e.g. harvest under full moon or use non-iron shears)`;
