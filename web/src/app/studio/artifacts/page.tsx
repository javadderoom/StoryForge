    ...
    try {
      ...
      if (json.success && json.data) {
        addArtifact({
          ...
          name: json.data.name || 'عتیقه باستانی',
          title: json.data.title || '',
          originEra: json.data.originEra || (isPersian ? 'عصر باستان' : 'Ancient Era'),
          rarity: json.data.rarity || 'epic',
          description: json.data.description || '',
          powers: json.data.powers || [isPersian ? 'نیروی جادویی پنهان' : 'Latent power'],
          curseOrCost: json.data.curseOrCost || undefined,
          attunementRules: json.data.attunementRules || undefined,
          currentHolderType: 'vault',
          currentHolderId: 'unknown',
          secretLore: json.data.secretLore || undefined,
        });
        notify.success(isPersian ? 'عتیقه اسطوره‌ای توسط هوش مصنوعی با موفقیت خلق شد' : 'Mythic relic generated by AI');
      } else {
        notify.error(isPersian ? 'خطا در خلق عتیقه' : 'Failed to generate artifact');
      }
    } catch {
      ...
      <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-3xl p-6 md:p-8 backdrop-blur-sm shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{isGeneratingAi ? (isPersian ? 'در حال خلق...' : 'Forging...') : (isPersian ? '⚡ دستیار هوش مصنوعی' : '⚡ AI Co-Pilot')}</span>
          </button>
