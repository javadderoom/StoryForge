  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      if (json.success && json.data) {
        addCreature({
          id: `creature_${Date.now().toString(36)}`,
          name: json.data.name || 'هیولای ناشناخته',
          speciesCategory: json.data.speciesCategory || 'monstrosity',
          dangerLevel: json.data.dangerLevel || 3,
          habitatLocationIds: [],
          behavioralTactics: json.data.behavioralTactics || '',
          weaknesses: json.data.weaknesses || [],
          resistances: json.data.resistances || [],
          harvestableLoot: json.data.harvestableLoot || [],
          loreDescription: json.data.loreDescription || '',
        });
        notify.success(isPersian ? 'پرونده زیستی توسط هوش مصنوعی خلق شد' : 'Bestiary creature generated by AI');
      } else {
        notify.error(isPersian ? 'خطا در خلق موجود' : 'Failed to generate creature');
      }
    } catch {
      ...
      <div className="bg-zinc-900/60 border border-zinc-800/80 rounded-3xl p-6 md:p-8 backdrop-blur-sm shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="px-3.5 py-2 rounded-2xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-rose-300 text-xs font-bold flex items-center gap-1.5 transition-all shadow-md disabled:opacity-50"
          >
            <Sparkles className={`w-3.5 h-3.5 ${isGeneratingAi ? 'animate-spin' : 'text-rose-400'}`} />
            <span>{isGeneratingAi ? (isPersian ? 'در حال خلق...' : 'Synthesizing...') : (isPersian ? '⚡ دستیار هوش مصنوعی' : '⚡ AI Co-Pilot')}</span>
          </button>
