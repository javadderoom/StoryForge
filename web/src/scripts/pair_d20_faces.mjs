...
loader.parse(arrayBuffer, '', (gltf) => {
  ...
  // Compute 2D bounding boxes and glyph properties for each face
  ...
  for (let i = 0; i < faceList.length; i++) {
    ...
    for (let j = i + 1; j < faceList.length; j++) {
      ...
      const fB = faceList[j];
      const dot = fA.center.dot(fB.center);
      if (dot < bestDot) {
        bestDot = dot;
        bestJ = j;
      }
    ...
    if (bestJ !== -1 && bestDot < -0.95) {
      used.add(i);
      used.add(bestJ);
      pairs.push({
        pair: pairs.length + 1,
        faceA: fA,
        faceB: faceList[bestJ],
        dot: Number(bestDot.toFixed(4))
      });
    }
  }

  console.log(`Found ${pairs.length} antipodal pairs:`);
  pairs.forEach(p => {
    console.log(`Pair ${p.pair}: Face ${p.faceA.id} (tri=${p.faceA.triCount}, w=${p.faceA.w}, double=${p.faceA.isDoubleDigit}) <---> Face ${p.faceB.id} (tri=${p.faceB.triCount}, w=${p.faceB.w}, double=${p.faceB.isDoubleDigit}) [dot=${p.dot}]`);
  });
});
